...
MSG_FORMAT = '[%(asctime)s] %(levelname)-5s --- %(message)s (%(filename)s : %(funcName)s : %(lineno)s)'
...
# getLogger - Main Call
## -----------------------------------------------------------------------------
def getLogger(name = 'noNameAssigned', shell=True, maya=_in_maya, nuke=_in_nuke, mobu=_in_mobu, file=None, level=INFO):
    '''
...
class Logger():
    ...
    def __init__(self, name , shell=True, maya=False, nuke=False, mobu=False, file=None, level=INFO):
        '''
        Init Logger
        '''
        ...
        # remove existing handlers to stop issues where maya wouldnt write to file
        ...
        # File:
        if file:
            file_hdlr = logging.FileHandler(file, mode="a")
